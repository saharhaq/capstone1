...
<th>Delete</th>
	...
	$sql = " SELECT  id, StudentID, Hours,Activity,Description,Approval, Entrytime  FROM Hours WHERE `Class code` = '".$_GET['TLD']."';";
	...
	$sql = " SELECT id, StudentID, Hours,Activity,Description,Approval, Entrytime FROM Hours  WHERE `studentid` = '".$number."';";
	$sql2 = " SELECT SUM(Hours) AS Hours FROM Hours where `studentid` = '".$number."' AND Approval='Approved'";

	}
	else if($search == "REF")	
	{
		$sql = "SELECT  id, StudentID,  Hours,Activity,Description,Approval, Entrytime FROM Hours ".
